...
console.log(m +"," + nbTweet );


var goodTweet = m[0]/nbTweet,
    badTweet  = m[1]/nbTweet,
    neutralTweet = (1 - (goodTweet + badTweet));

console.log(goodTweet +"," + neutralTweet +"," + badTweet);

$(listBarStat[0]).attr("aria-valuenow", goodTweet * 100).attr("style","width: "+goodTweet* 100+"%").html(Math.round(goodTweet * 100) + "% <span class='glyphicon glyphicon-heart-empty' aria-hidden='true'>");
$(listBarStat[2]).attr("aria-valuenow", badTweet * 100).attr("style","width: "+badTweet * 100+"%").html(Math.round(badTweet* 100) + "% <span class='glyphicon glyphicon-warning-sign' aria-hidden='true'>");
$(listBarStat[1]).attr("aria-valuenow", neutralTweet * 100).attr("style","width: "+neutralTweet * 100 +"%").html(Math.round(neutralTweet * 100) + "% <span class='glyphicon glyphicon-thumbs-up' aria-hidden='true'>");
});
